...
namespace HoloToolkit.Unity.InputModule.Utilities.Interactions
{
    ...
    /// </summary>
    public class NewTwoHandManipulation : MonoBehaviour, IInputHandler, ISourceStateHandler
    {
        [SerializeField]
        ...
        private void OnManipulationEnded()
        {
            ...
            for (int i=0; i<3; i++) {
                ...
                // Take remainder of division by 90 degrees
                float remainder = angles[i] % 90;

                // If remainder less than 45, round down
                if (remainder <= 45 && remainder>0) {
                    angles[i] = angles[i] - remainder;
                }

                // If remainder greater than 45, round up
                else if (remainder>45 && remainder<90) {
                    angles[i] = angles[i] + (90 - remainder);
                }
            }
